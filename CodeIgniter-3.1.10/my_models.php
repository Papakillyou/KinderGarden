<?php die();

/**
 * Add you custom models here that you are loading in your controllers
 *
 * <code>
 * $this->site_model->get_records()
 * </code>
 * Where site_model is the model Class
 *
 * ---------------------- Models to Load ----------------------
 * <examples>
 *
 * @property membership_model $membership_model
 * @property site_model       $site_model
 * @property data_model       $data_model
 * @property products_model   $products_model
 * @property film_model       $film_model
 * @property film_model1      $film_model1
 * @property ClassModel       $classmodel
 * @property LessonModel       $lessonmodel
 * @property WorkerModel       $workerModel
 * @property WorkerSalaryModel       $workerSalaryModel
 * @property TeacherLessonModel       $teacherLessonModel
 * @property InventoryModel       $inventoryModel
 * @property StudentModel       $studentModel
 * @property StudentPayModel       $studentPayModel
 */
class my_models
{
}

// End my_models.php
